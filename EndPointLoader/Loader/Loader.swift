...
protocol LoaderProtocol {
    func get(_ url: URL, ignoreCache: Bool, json: JSON?, timeoutInterval: TimeInterval, completionHandler: @escaping (Result<Data, Error>) -> Void)
...
public class Loader: LoaderProtocol {
    ...
    public func get(_ url: URL, ignoreCache: Bool, json: JSON?, timeoutInterval: TimeInterval, completionHandler: @escaping (Result<Data, Error>) -> Void) {
        let request = requestObject(withUrl: url, parameter: json)
        execute(request, completionHandler)
    }

    public func post(_ url: URL, body: Data? = nil, headers: HTTPHeaders? = nil, completionHandler: @escaping (Result<Data, Error>) -> Void) {
        let urlRequest = requestObject(withUrl: url, httpMethod: .POST, jsonBodyData: body)
        return execute(urlRequest, completionHandler)
    }
    
   public func put(_ url: URL, body: Data? = nil, completionHandler: @escaping (Result<Data, Error>) -> Void) {
